...
def init_db():
    ...
    # Labels table (learned from user)
...
def parse_time(text):
    ...
    # Direct time formats
    try:
        # "2026-07-25 14:30" or "2026-07-25"
        if len(text) == 10:  # date only
            dt = datetime.strptime(text, "%Y-%m-%d")
            return dt.replace(hour=9, minute=0)  # default 9am
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Check for delete command in reply
    ...
    # Check for reminder request in reply
    ...
    if "label:" in text.lower():
        ...
        if label_part:
            label = label_part
            # Check if we know this label
            delay = get_label(user_id, label)
            if delay is None:
                # Ask user to define it
    ...
    # If label is known, auto-set reminder
    if label:
        delay = get_label(user_id, label)
        if delay:
...
async def handle_label_definition(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle user defining a new label duration"""
    user_id = update.effective_user.id
    text = update.message.text.lower().strip()
    
    if "pending_label" not in context.user_data:
        return  # Not waiting for label definition
    ...
    if len(parts) == 2:
        try:
            ...
            if seconds > 0:
                ...
                # Now save the original message with this label
...
async def last_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not results:
        await update.message.reply_tex
